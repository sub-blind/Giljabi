...
2026년 9월 21일 기준. 회원·인증 세션 저장, 프런트 카카오 로그인 상태, 계정별 코스 생성·목록·조회·삭제와 회원 탈퇴를 구현했다. 방문 체크와 메모는 현재 브라우저에만 저장한다.
...
토큰 갱신은 `SELECT FOR UPDATE`로 같은 세션을 잠근다. 먼저 갱신된 세션은 폐기 상태가 되어 재사용을 거절한다. 새 토큰 저장에 실패하면 기존 세션 폐기도 롤백한다. 폐기·만료된 세션은 접근 토큰도 거절한다. 다른 기기의 별도 세션까지 로그아웃하지 않는다. 회원 탈퇴는 로그인 유지용 토큰 해시를 재검증한 뒤 사용자 행을 삭제하며 외래키 연쇄 삭제로 모든 세션·코스·코스 장소를 함께 제거한다. 만료·폐기 행의 주기적 정리는 이후 운영 단계다.
