...
상위 컴포넌트의 `useReducer`로 여행 상태를 관리하고 입력 방식은 별도 `useState`로 관리한다. 입력 방식의 초기값은 ‘지도로 찾기’이며 탭 전환으로 여행 문장과 편집 중인 조건을 초기화하지 않는다. 편집 중인 조건과 결과에 적용한 조건을 분리한다. 새 검색이 성공하면 이전 선택을 초기화하고 알린다. 실패하면 이전 결과와 적용 조건을 유지한다.
...
초기 상태 확인은 Render 인스턴스를 미리 깨우기 위한 보조 요청이며 검색을 잠그는 조건으로 사용하지 않는다. 사용자는 연결 확인 중에도 지도·문장·사진 검색을 시작할 수 있다. 여행 검색과 사진 조회가 8초 이상 걸리면 현재 확인 중인 작업과 취소 버튼을 보여준다. 취소해도 작성한 문장·선택 조건·기존 결과는 유지한다. 사진 검색이 정상 완료됐지만 키워드 결과가 비면 같은 지역을 키워드 없이 다시 조회하는 버튼을 제공한다.

## 4. 세 화면

### 여행 입력

‘지도로 찾기’, ‘문장으로 찾기’, ‘사진으로 찾기’ 세 탭으로 입력 방식을 선택하며 한 번에 한 화면만 표시한다. 처음에는 지도 탭을 열고 강원도 18개 시군 경계와 장소 유형을 보여준다. 직접 검색은 지역·장소 유형만 사용하며 별도의 선호 키워드 입력을 받지 않는다. 시군 선택지는 서버가 조회한 실제 국문 관광정보 코드 목록과 일치시킨다.

문장 탭은 입력과 예시 선택을 제공하며 해석 실행 버튼을 눌렀을 때만 요청한다. 성공하면 같은 검색 영역에서 지도 탭으로 전환하고 편집 영역에 초점을 옮긴다. 사용자는 지역·관심 유형을 고치고 AI가 문장에서 반영한 키워드·분위기를 읽기 전용으로 확인한다. 확인할 수 없는 조건은 사실 조건과 구분해서 표시한다. 상태 응답의 `aiReady`가 false이면 문장 해석 준비 중 안내와 직접 지도 선택 버튼을 제공한다.
...
실제 사진·이름·분류·주소와 상세 조회를 제공한다. 사진이 없으면 누락 안내를 표시한다. 개발자 검증용 근거 문구는 후보 카드에 노출하지 않는다. 상세에서는 관광 API가 제공한 전화·운영시간·휴무일·주차·요금·메뉴 중 값이 있는 항목만 보여준다. 최대 세 장소를 고르고 특정 장소를 다른 후보로 바꿀 수 있다. 대체 후보가 없으면 추가 조회 또는 조건 변경을 안내한다. 후보 표시나 선택을 위해 LLM을 호출하지 않는다.

명소 유형에 섞인 화장실·주차장·관광안내소는 이름 기준으로 여행 후보에서 제외한다. 이 필터는 편의시설 후보를 줄이는 규칙이며 모든 장소의 목적 적합성을 보장하는 학습 모델은 아니다.

### 코스 확인

서버에서 다시 확인한 실제 장소, 소개 원문과 대조한 AI 단서 또는 확인된 사실, 위·아래 순서 변경, 좌표 마커, 카카오맵 길찾기와 저장을 제공한다. 여행 시작 후에는 다음 미방문 장소와 빠른 현장 정보, 방문 완료, 메모를 먼저 보여준다. 순서 변경에는 LLM을 호출하지 않는다. 첫 장소는 목적지 길찾기, 이후 장소는 직전 코스 장소를 출발지로 전달한다. 좌표가 없는 목적지는 장소 검색으로 연결한다. 자동차 이동 확인을 실행하면 카카오모빌리티의 예상 시간·거리·도로 경로를 별도로 표시한다.

여행 시작은 로그인 없이 가능하다. 비회원 최근 코스와 방문 기록은 브라우저에 저장하고, 로그인 사용자가 ‘코스 저장’을 실행하면 계정별 코스를 PostgreSQL에도 저장한다. 방문 기록은 `storyroute.journey.v1:{정렬한 장소 ID 집합}` 키로 구분하고 `version`, `placeIds`, `startedAt`, `visitedIds`, `notes`만 저장한다. 메모는 장소별 최대 500자이며 명시적인 저장 버튼으로 저장한다. 저장 실패 시 성공·방문 상태를 갱신하지 않는다. 순서 변경은 기록의 소속을 바꾸지 않고 장소 구성이 다르면 다른 기록을 조회한다. 방문 완료·메모의 계정 동기화는 제공하지 않는다.

## 5. 반응형 배치

| 폭 | 배치 |
|---|---|
| 320~767px | 작은 강원도 지도와 18개 시군 버튼, 장소 유형, 전체 폭 검색 버튼, 카드 한 열, 선택 목록·하단 코스 진행 버튼, 여행 중 하단 길찾기, 상세 바텀시트 |
| 768~1023px | 강원도 지도와 지역 설명, 장소 유형, 카드 두 열, 선택 목록 별도 행, 코스 지원 영역 한 열 |
| 1024px 이상 | 강원도 지도와 지역 설명을 나란히 배치, 후보 목록·선택 목록을 나란히 배치, 자동차 이동과 코스 지도를 두 열로 배치 |
...
사진 요청은 `city`(null 또는 강원 시군), `keyword`(최대 40자), `page`(조회 시작 페이지, 1~5)를 받는다. 서버는 원본 API를 페이지당 24장씩 조회하며 제목·촬영지·시군이 다른 묶음 여섯 개를 확보할 때까지 한 요청에서 최대 세 페이지를 보충한다. 원본 결과가 끝나거나 다섯 번째 페이지에 도달하면 멈춘다. 촬영지·이미지 주소 검증을 통과한 사진은 12장으로 자르지 않고 모두 반환한다. 응답의 `page`는 마지막으로 조회에 성공한 원본 페이지이며 추가 조회는 `page + 1`에서 시작한다. 보충 도중 외부 호출이 실패하면 이미 확보한 사진과 마지막 성공 페이지를 반환하고 안내를 붙인다. 첫 호출 실패는 오류 응답으로 처리한다. 묶음 수는 실제 이미지 로드 성공을 보장하지 않는다.

응답 사진의 `id`는 `photo_` 접두사이며 실제 관광 장소 ID가 아니다. 촬영지·촬영자·키워드·이미지 주소를 표시한다. 사진 속 정확한 장소와 좌표를 단정하지 않고 ‘이 지역 장소 찾기’로 해당 시군의 관광 후보를 새로 검색한다.

사진 화면은 제목·촬영지·시군이 같은 사진을 한 카드로 묶고, 같은 이미지 주소는 묶음 안에서 한 번만 표시한다. 사진을 넘기면 현재 사진의 촬영자도 함께 바뀐다. 이 묶음은 실제 관광 장소의 동일성을 검증한 결과가 아니다. 이미지 로드가 실패하면 해당 주소를 제외하고 같은 묶음의 다른 사진을 표시한다. 남은 사진이 없는 카드는 숨기고 실패한 사진 수를 짧게 안내한다. 모든 사진이 실패하면 조건 변경 안내를 제공한다. 현재 남아 있는 촬영지 수·사진 수를 표시하며 PC 세 열·태블릿 두 열·모바일 한 열로 배치한다. 카드가 한두 개여도 PC에서 전체 폭으로 늘어나지 않는다. 추가 조회 범위가 남으면 ‘다른 풍경 더 보기’를 제공한다. 추가 조회는 기존 묶음·선택 사진·실패 기록을 유지한다. 새 검색이 성공하면 사진 선택과 실패 기록을 초기화하고, 실패하면 이전 결과를 보존한다.
...
비회원 코스 저장에는 버전·ID 순서·최종 조건·저장 시각만 저장하고 여행 기록은 별도 키로 저장한다. 새 저장 키는 `storyroute.day-trip.v2`다. 기존 v1의 강릉 조건은 강원도·강릉시로 변환해 읽고 원래 저장은 삭제하지 않는다. 로그인 사용자의 계정 코스에는 제목·ID 순서·최종 조건을 PostgreSQL에 저장하며 최근 20개를 조회·재열기·삭제할 수 있다. 두 저장 모두 코스를 열 때 서버에서 실제 관광 정보를 다시 조회한다. 방문 완료와 메모는 현재 브라우저에만 남는다.
