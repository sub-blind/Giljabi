...
2026년 9월 18일 · SQLAlchemy 모델·초기 Alembic 마이그레이션 구현 및 로컬 DB 적용 완료.

사용자별 코스·방문 체크·개인 메모와 로그인 유지에 필요한 테이블을 생성했다. 공개 리뷰·사진 업로드는 이후에 별도로 설계한다. 카카오 로그인은 `users`와 `auth_sessions`에 연결했다. 현재 화면의 코스·방문 체크·메모는 브라우저에 저장하며 코스·메모 API의 DB 저장은 이후 단계다.
...
`intent`는 JSON 객체만 허용하며 기존 FastAPI 조건 모델로 검증한다. 강원도 당일 여행 조건만 저장하고 AI 응답 원문·비밀 값은 넣지 않는다. 저장·조회·수정·삭제는 로그인 사용자와 `user_id`가 일치하는 경우만 허용한다. 수정 요청의 버전이 현재 버전과 다르면 충돌을 안내하고, 성공하면 버전을 증가시킨다.
...
- 저장 시 실제 관광 API로 재검증한다. 관광 소개·사진 원본·AI 설명을 대량 저장하지 않고 코스를 열 때 조회한다. 조회 실패로 기존 저장 장소를 자동 삭제하지 않는다.

장소 삭제·교체는 해당 장소의 기록도 삭제한다는 것을 화면에서 알린다. 순서 변경은 행을 삭제하고 다시 만들지 않아 방문 기록을 유지한다.
...
코스 생성·장소 변경·코스 버전 갱신은 하나의 트랜잭션으로 처리한다. 메모·방문 기록 수정도 부모 코스의 버전을 확인하고 갱신해 동시에 수정한 내용을 조용히 덮어쓰지 않는다. 타인의 장소 ID를 보내도 소유권 검증을 통과하지 못하면 저장하지 않는다.

테이블 생성과 변경은 Alembic으로 관리한다. `app/models.py`와 초기 변경 `20260918_01`을 구현하고 로컬 Docker PostgreSQL에 적용했다. 직접 SQL로 수정 시각을 변경할 때는 `updated_at`을 명시해야 하며 SQLAlchemy 모델을 통한 수정에서는 자동 갱신된다. 소유권 검사·버전 충돌 처리·실제 관광 ID 확인은 이후 계정 저장 API에서 구현해야 한다.
