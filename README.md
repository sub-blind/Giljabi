...
- [데이터 모델과 ERD](docs/DATA_MODEL.md)
...
| 영역 | 구성 |
|---|---|
| 프런트 | React 19·TypeScript·Next.js 16·CSS Modules |
...
| 로그인·서버 DB | 카카오 로그인, PostgreSQL 회원·세션·계정별 코스 저장·조회·삭제, 회원 탈퇴 |
...
2026년 9월 21일 코드·로컬 검증과 공개 상태 API 확인 기준이다. 구현 여부, 자동 검증, 실제 외부 연결, 모바일 브라우저 확인을 구분하며 상세 근거는 [검증 기록](docs/VALIDATION.md)에 남긴다.
...
| 문장 해석·AI 근거 선택 | 실제 Responses API 연결·고정 예시 평가 확인 | 조건 해석 10개·춘천 실제 후보 18개·두 장소 소개 원문 대조, AI 실패 시 직접 조건 편집 유지 |
| 카카오 로그인·계정 저장 | 구현·로컬 실제 로그인 확인 | 회원·세션 DB 저장, 계정별 코스 저장·최대 20개 목록·재검증 후 열기·삭제, 회원 탈퇴 연쇄 삭제 |
| PostgreSQL 기반 연결 | 실제 로컬 연결·읽기·쓰기 확인 | DB 연결 확인 API, 연결 수·시간 제한·오류 숨김, 회원·세션·코스 저장 |
| 코스 공유·방문 기록 동기화 | 구현 전 | 방문 체크·메모는 현재 브라우저에만 저장 |
| 공개 운영 주소 | Vercel·Render 상태 응답 확인 | `storyroute.vercel.app` 화면과 `storyroute-api.onrender.com` API, 첫 유휴 요청 지연 안내·취소 제공 |
...
접근 토큰도 DB 세션의 만료·폐기 여부를 확인한다. 토큰 갱신은 행 잠금으로 같은 세션의 중복 갱신을 막고, 기존 세션 폐기와 새 세션 생성을 함께 처리한다. 로그아웃 시 해당 세션을 폐기한다. 이전 메모리 방식으로 발급한 쿠키는 다시 로그인해야 한다. 프런트는 로그인 회원의 닉네임·로그아웃·내 코스 목록을 제공한다. 계정 코스에는 장소 ID·순서·검색 조건만 저장하고 다시 열 때 관광 API로 현재 정보를 검증한다. 회원 탈퇴는 로그인 유지용 토큰을 재검증한 뒤 회원·세션·계정 코스를 연쇄 삭제한다. 방문 체크와 메모는 브라우저 저장을 유지하며 기기 기록 삭제에서 별도로 제거한다.
